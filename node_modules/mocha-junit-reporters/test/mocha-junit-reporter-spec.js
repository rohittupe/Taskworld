...
var debug = require("debug")("mocha-junit-reporters:tests");

chai.use(chaiXML);

describe("mocha-junit-reporters", function() {
  var runner;
  var filePath;
  var MOCHA_FILE;
  ...
  describe('when "useFullSuiteTitle" option is specified', function() {
    ...
    it('generates full suite title separated by "suiteTitleSeparatedBy" option', function() {
      var reporter = configureReporter({
  ...
  describe('when "outputs" option is specified', function() {
    it("adds output/error lines to xml report", function() {
      ...
      expect(testsuites[0].testsuite[1].testcase[0]._attr.name).to.equal(
        test.fullTitle()
      );
      expect(testsuites[0].testsuite[1].testcase[1]).to.have.property(
        "system-out",
        "hello\nworld"
      );
      expect(testsuites[0].testsuite[1].testcase[2]).to.have.property(
        "system-err",
        "typical diagnostic info\nall is OK"
      );
  ...
  describe("Output", function() {
    ...
    it('uses "Root Suite" by default', function() {
      runner.startSuite({ title: "", root: true, suites: [1] });
      runner.end();
      expect(testsuites[0].testsuite[0]._attr).to.have.property(
        "name",
        "Root Suite"
      );
    });
  ...
  describe('Feature "Configurable classname/name switch"', function() {
    ...
    it("should generate valid testCase for testCaseSwitchClassnameAndName default", function() {
      ...
      expect(testCase.testcase[0]._attr.name).to.equal(
        mockedTestCase.fullTitle()
      );
      expect(testCase.testcase[0]._attr.classname).to.equal(
        mockedTestCase.title
      );
    });
  ...
  describe("XML format", function() {
    var suites = [
      { testsuite: { title: "", root: true, suites: [2], tests: [0] } },
      {
        testsuite: { title: "Inner Suite", suites: [1], tests: [1] },
        pass: [{ title: "test", fullTitle: "Inner Suite test" }]
      },
      {
        testsuite: { title: "Another Suite", suites: [1], tests: [1] },
        fail: [
          {
            title: "fail test",
            fullTitle: "Another Suite fail test",
    ...
    it("generates Jenkins compatible XML when in jenkinsMode", function() {
      ...
      var xsd = fs.readFileSync(
        path.join(__dirname, "resources", "jenkins-junit.xsd")
      );
    ...
    describe("Jenkins format", function() {
      ...
      it("generates Jenkins compatible classnames and suite name", function() {
        ...
        expect(reporter.suites[0].testsuite[0]._attr.name).to.equal(
          suites[0].testsuite.title
        );
        expect(reporter.suites[0].testsuite[1].testcase[0]._attr.name).to.equal(
          suites[0].pass[0].title
        );
        expect(
          reporter.suites[0].testsuite[1].testcase[0]._attr.classname
        ).to.equal(suites[0].testsuite.title);
        expect(reporter.suites[1].testsuite[0]._attr.name).to.equal(
          suites[0].testsuite.title + "." + suites[0].suites[0].testsuite.title
        );
        expect(reporter.suites[1].testsuite[1].testcase[0]._attr.name).to.equal(
          suites[0].suites[0].fail[0].title
        );
        expect(
          reporter.suites[1].testsuite[1].testcase[0]._attr.classname
        ).to.equal(
          suites[0].testsuite.title + "." + suites[0].suites[0].testsuite.title
        );
      });
